...
class Color:
    ...
    def _rgb_euclidean_distance(self, color_obj):
        assert type(color_obj) == Color

        return np.round(np.linalg.norm(self.RGB - color_obj.RGB), 3)

    def _delta_E(self, color_obj):
    ...
    def delta_E(self, color_objs):
        if isinstance(color_objs, (list, np.ndarray)):
            delta_E_arr = []
            for c in color_objs:
                delta_E_arr.append(self._delta_E(c))
            return np.array(delta_E_arr)
        else:
            return self._delta_E(color_objs)

    def rgb_euclidean_distance(self, color_objs):
        if isinstance(color_objs, (list, np.ndarray)):
            euclidean_distances = []
            for c in color_objs:
                euclidean_distances.append(self._delta_E(c))
            return np.array(euclidean_distances)
        else:
            return self._rgb_euclidean_distance(color_objs)

    def contrast(self, color_objs):
        if isinstance(color_objs, (list, np.ndarray)):
            contrasts = []
            for c in color_objs:
                contrasts.append(self._delta_E(c))
            return np.array(contrasts)
        else:
            return self._contrast(color_objs)